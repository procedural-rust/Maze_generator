...
pub struct Room {
	...
	pub dimensions: Point, //The dimension of the room.
}
...
impl Room {
    ...
    pub fn contains_cell(my_room: Room, cell: Point) -> bool {
        let in_x_dim = (my_room.base_point.row <= cell.row) && (cell.row <= (my_room.base_point.row + my_room.dimensions.row-1));
        let in_y_dim = (my_room.base_point.col <= cell.col) && (cell.col <= (my_room.base_point.col + my_room.dimensions.col-1));
    ...
    pub fn check_intersection(first_room: Room, second_room: Room) -> bool {
        ...
        let second_room_top_left = Point::init(second_room.base_point.row,second_room.base_point.col + second_room.dimensions.col - 1);
        if Room::contains_cell(first_room,second_room_top_left) {
            return true;
        }
        let second_room_bottom_right = Point::init(second_room.base_point.row + second_room.dimensions.row - 1,second_room.base_point.col);
        ...
        let first_room_top_left = Point::init(first_room.base_point.row,first_room.base_point.col + first_room.dimensions.col - 1);
        if Room::contains_cell(second_room,first_room_top_left) {
            return true;
        }
        let first_room_bottom_right = Point::init(first_room.base_point.row + first_room.dimensions.row - 1,first_room.base_point.col);
        if Room::contains_cell(second_room,first_room_bottom_right) {
            return true;
        }
        //Note that we don't check if the top right cell of a room is contained in the other since if it were true, so would one of the above conditions.
        //Second case: the two rooms form a cross,
        if (second_room.base_point.row < first_room.base_point.row) && (second_room.base_point.row + second_room.dimensions.row - 1 > first_room.base_point.row){
            if (first_room.base_point.col < second_room.base_point.col) && (first_room.base_point.col + first_room.dimensions.col - 1 > second_room.base_point.col){
                return true;
            }
        } else if (first_room.base_point.row < second_room.base_point.row) && (first_room.base_point.row + first_room.dimensions.row - 1 > second_room.base_point.row) {
            if (second_room.base_point.col < first_room.base_point.col) && (second_room.base_point.col + second_room.dimensions.col - 1 > first_room.base_point.col) {
                return true;
            }
        }
...
pub fn create_dungeon(my_rows: usize, my_columns: usize, wrap: usize, method: GenerationType, num_rooms: usize, prune_dead_ends_ratio: f64, outside_exits: bool) -> Result<Dungeon,DungeonError>{
    ...
    let dungeon_bounds = Room::init(Point::init(0,0),Point::init(my_rows,my_columns));
    for _i in 0..num_rooms {
        let new_room_dim = Point::init(rand::thread_rng().gen_range(ROOM_MIN_WIDTH,ROOM_MAX_WIDTH),rand::thread_rng().gen_range(ROOM_MIN_HEIGHT,ROOM_MAX_HEIGHT));
    ...
    for room in my_rooms.iter() {
        ...
        for i in 0..(room.get_width()){
            for j in 0..(room.get_height()){
...
mod tests {
    ...
    fn room_contain_cell_test() {
        //A room which is a single cell
        let single_cell_room = Room::init(Point::init(0,0),Point::init(1,1));
        ...
        //a room with different x and y dimensions.
        let test_room = Room::init(Point::init(2,3),Point::init(2,3));
        for i in 2..((2+1) + 1) {
            for j in 3..((3+ 2) + 1) {
    ...
    fn room_intersection_test() {
        let single_cell_room = Room::init(Point::init(0,0),Point::init(1,1));
        let room_1 = Room::init(Point::init(0,0),Point::init(3,3));
        let room_2 = Room::init(Point::init(3,0),Point::init(3,3));
        let room_3 = Room::init(Point::init(0,3),Point::init(3,3));
        let room_4 = Room::init(Point::init(3,3),Point::init(3,3));
        let room_5 = Room::init(Point::init(2,2),Point::init(2,2));
        let room_6 = Room::init(Point::init(1,1),Point::init(4,4));

        let room_7 = Room::init(Point::init(1,1),Point::init(1,3));
        let room_8 = Room::init(Point::init(1,3),Point::init(3,1));
        ...
        //check room intersections with multiple rooms
    ...
    #[test]
    fn create_room_test() {
        let dimensions_1 = Point::init(1,1);
        let dimensions_2 = Point::init(6,6);
        let dimensions_3 = Point::init(8,8);
