...
impl Compass {
    ...
    //add_dir
    //Purpose:
    //  Adds the given direction to the compass.
    ...
    //remove_dir
    //Purpose:
    //  Removes the given direction from the compass.
    ...
    //get_exits
    //Purpose:
    //    Returns a Vec of the directions a compass has.
...
impl Maze {
    ...
    //erase_dead_end
    //Purpose:
    //    Erases a dead end by removing the dead and and all paths that lead to it from a intersection.
    //    Returns true if successful, returns false if the given cell was not a dead end.
    //Pre-Conditions:
    //    The maze must be a dead end at the given point (verified).
    //
    pub fn erase_dead_end(&mut self,dead_end: Point) -> bool{
        let mut found_junction = false; //found a cell with more at least 3 exits.
        let mut current_cell = dead_end;
        if self.maze_matrix[current_cell.row][current_cell.col].get_exits().len() != 1 {
            //The cell passed was not a dead end.
            return false;
        }
        while !found_junction {
            ...
            match get_cell_in_direction(self.rows, self.columns, current_cell.row, current_cell.col, only_exit, MAX_SQUARE_WRAP) {
                Some(cell) => {
                    current_cell = cell;
                    self.maze_matrix[current_cell.row][current_cell.col].remove_dir(only_exit.reverse());
                    let current_exits = self.maze_matrix[current_cell.row][current_cell.col].get_number_of_exits();
                    if current_exits > 1 {
                        found_junction = true;
                    } else if current_exits == 0 {
                        found_junction = true;
                    }
                },
                None => found_junction = true,
            }
        }
    ...
    //add_wall
    //Purpose:
    //    Add a wall in the maze.
    //Notes:
    //    Each wall has two ways to refer to it since it is a barrier between two cells. The compass in both cells must be modified.
    //    If the cell on the otherside is a cell outside the map, then only the first wall is removed since the other doesn't exist.
    pub fn add_wall(&mut self,my_wall: Wall, wrap: usize){
        let current_cell = my_wall.cell;
        self.maze_matrix[current_cell.row][current_cell.col].remove_dir(my_wall.dir);
        match get_cell_in_direction(self.rows, self.columns, current_cell.row, current_cell.col, my_wall.dir, wrap) {
            Some(cell) => self.maze_matrix[cell.row][cell.col].remove_dir(my_wall.dir.reverse()),
            None => return,
        };
    }
    ...
    //remove_wall
    //Purpose:
    //    Remove a wall in the maze.
    //Notes:
    //    Each wall has two ways to refer to it since it is a barrier between two cells. The compass in both cells must be modified.
    //    If the cell on the otherside is a cell outside the map, then only the first wall is added since the other doesn't exist.
    pub fn remove_wall(&mut self,my_wall: Wall, wrap: usize){
        let current_cell = my_wall.cell;
        self.maze_matrix[current_cell.row][current_cell.col].add_dir(my_wall.dir);
        match get_cell_in_direction(self.rows, self.columns, current_cell.row, current_cell.col, my_wall.dir, wrap) {
            Some(cell) => self.maze_matrix[cell.row][cell.col].add_dir(my_wall.dir.reverse()),
            None => return,
        };
    }
    ...
    while walls.len() != 0 { //while there are still walls.
        ...
        if let Some(cell) = next_cell {
            if !check_matrix[cell.row][cell.col] && bitmask[cell.row][cell.col] {//if there is a cell on the other side and it hasn't been visited yet and is a valid cell.
                ...
                path_matrix[current_cell.row][current_cell.col].add_dir(current_dir);
                path_matrix[cell.row][cell.col].add_dir(current_dir.reverse());
            }
        }
    }
    return path_matrix;
...
pub fn get_cell_in_direction(max_rows: usize, max_cols: usize, row: usize, col: usize, dir: Direction, wrap: usize) -> Option<Point> {
    let mut new_row = row;
    let mut new_col = col;
    ...
    if wrap_level > wrap { //if the move requires wrapping, but the map doesn't support it, then we are outside the map.
        return None;
    }else {
...
fn bias_recursive_backtrack_algorithm(my_rows: usize, my_columns: usize, wrap: usize, straightness: f64, bitmask: &Vec<Vec<bool>>, starting_points: Vec<Point>) -> Vec<Vec<Compass>> {
    ...
    for start_point in starting_points {
        check_matrix[start_point.row][start_point.col] = true;
        cells.push(start_point); // these cells will be where we start making the maze
    }
    ...
    while cells.len() != 0 { //while there are still cells.
        ...
        for a_direction in all_directions.iter() { // get valid moves
            if let Some(cell) = get_cell_in_direction(my_rows,my_columns,current_cell.row,current_cell.col,*a_direction,wrap) {//if we can move in that direction
                if (check_matrix[cell.row][cell.col] == false) && bitmask[cell.row][cell.col] { // and the cell has not yet been used and is valid, then the direction is valid.
                    nearby_cells.push((a_direction,cell)); //cells we can move to
                    if *a_direction == current_direction {
                        continue_in_current_direction.push((a_direction,cell)); //the cell that is in the same direction we are already moving
                    }else{
                        continue_in_other_direction.push((a_direction,cell)); //the cells that are not in the same direction we are already moving.
                    }
                }
            }
        }

        if nearby_cells.len() == 0 { //If we can't move anywhere, then pop the cell of the stack since it is a deadend.
            cells.pop().unwrap();
            ...
            }else{ //otherwise we must change direction, so pick any other valid direction with equal probability
                choice = rand::thread_rng().gen_range(0, continue_in_other_direction.len());
                next_cell_data = continue_in_other_direction[choice];
            }
            ...
            path_matrix[current_cell.row][current_cell.col].add_dir(current_direction);
            path_matrix[next_cell.row][next_cell.col].add_dir(current_direction.reverse());
        ...
        for col in 0..my_columns {
            if !check_matrix[row][col] && bitmask[row][col] { // if the current square is not already in the maze, and allowed by the bitmask.
                ...
                while check_matrix[current_square.row][current_square.col] != true { //preform a loop erased random walk
                    let neighbor_data = get_random_neighbor(my_rows,my_columns,current_square.row,current_square.col,wrap, &bitmask); //move to an random adjacent cell.
                ...
                for k in 0..(trail.len()-1) { // note that since we start at a square not already in the maze tha trail is at least 2.
                    ...
                    path_matrix[trail[k].row][trail[k].col].add_dir(trail_directions[k]);
                    path_matrix[trail[k+1].row][trail[k+1].col].add_dir(trail_directions[k].reverse());
                }
            }
        }
    }
...
//Pre-Conditions:
//    The point (row, col) is such that 0 <= row < max_rows and 0 <= col < max_cols. (Not verified)
//    The conditions are such that the cells has a neighbor.
//    If there is no wrapping and max_rows = max_cols = 1 the function will crash. (Not verified)
...
mod tests {
    ...
    //tests that get_starting_points correctly accounts for level wrap-around.
    #[test]
    fn get_starting_points_test() {
        //Four separate points with no wrap, should have four separate starting points.
        ...
        assert!(my_points.len() == 4);
        //Four separate points with wrap on one side, should have two seperate starting points.
        my_points = get_starting_points(3,3,RING_SQUARE_WRAP,&mask.clone());
        assert!(my_points.len() == 2);
        //Four separate points with full wrap, should have one starting point.
        my_points = get_starting_points(3,3,MAX_SQUARE_WRAP,&mask);
        assert!(my_points.len() == 1);
        //One contiguous map with no wrap, should have one starting point.
        mask = vec![vec![true,true,true],vec![true,true,true],vec![true,true,true]];
        my_points = get_starting_points(3,3,NO_SQUARE_WRAP,&mask);
        assert!(my_points.len() == 1);
        //Two separate parts with no wrap, should have two saperate starting points.
        ...
        assert!(my_points.len() == 2);
        //Two separate parts with wrap on the side that would not connect them, should have two starting points.
        my_points = get_starting_points(3,3,RING_SQUARE_WRAP,&mask);
        assert!(my_points.len() == 2);
        //A ring map with no wrap, should have one starting point.
        ...
        my_points = get_starting_points(3,3,NO_SQUARE_WRAP,&mask.clone());
        assert!(my_points.len() == 2);
        //Two separate parts with wrap on the side that would connect them, should have one starting point.
        my_points = get_starting_points(3,3,RING_SQUARE_WRAP,&mask);
        assert!(my_points.len() == 1);
    }
